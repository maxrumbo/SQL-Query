...
CREATE OR ALTER PROCEDURE GetCompaniesWithSecondCharA
AS
BEGIN
    SELECT CustomerName
    FROM Customers
    WHERE SUBSTRING(CustomerName, 2, 1) = 'a'
...
CREATE OR ALTER PROCEDURE GetAllCustomerDataOrdered
AS
BEGIN
    SELECT 
        c.CustomerName,  -- Ganti CompanyName dengan CustomerName
        c.ContactName,
    ...
    GROUP BY 
        c.CustomerName, c.ContactName
    ORDER BY 
        c.CustomerName ASC, COUNT(o.OrderID) ASC;
END;
...
CREATE OR ALTER PROCEDURE GetEmployeeDataWithAge
AS
BEGIN
    SELECT 
        EmployeeID,
...
-- Membuat atau mengubah stored procedure
...
BEGIN  
    ...
    JOIN   
        ...
        (@ContactName IS NULL OR Customers.ContactName = @ContactName); 
		END  

-- Menjalankan prosedur:
EXEC sp_Exercise4;  

-- Atau dengan filter nama kontak:
EXEC sp_Exercise4 'Karin Josephs';
